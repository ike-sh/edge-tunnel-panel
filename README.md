...
多公网入口不是 B 侧自动负载均衡：外部客户端连接哪台 A，就从哪台 A 的公网地址进入。`weight` 用于输出排序和 PRIMARY / BACKUP 推荐，不代表自动按权重分流；真正自动负载均衡需要客户端、DNS 或外部 LB 配合。手动切换入口请使用：`公网入口列表管理 -> 切换主公网入口`。
...
`生成转发入口输出` 的 TXT 清单会标注 `TCP+UDP`，并把 TCP 探测和 UDP 参考探测分开显示，避免把 EasyTier peer 协议和业务转发协议混淆。
