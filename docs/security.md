...
本页汇总 Leikwan Toolkit 1.3.3 的敏感信息和高危操作边界。
...
1.3.3 起，脚本会根据真正的角色信号做检测：relay service 或 `ROLE=leikwan-relay` 表示 B 角色；entry service、`ROLE=cloud-entry` 或 entry env 表示 A 角色。`entries.tsv` 不再作为 entry 判据，`forwards.tsv` 也不能单独决定角色。B 菜单在 A 机器执行高危操作、A 菜单在 B 机器执行高危操作时会先提示角色不匹配，交互模式默认不继续。
