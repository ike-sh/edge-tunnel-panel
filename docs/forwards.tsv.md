...
默认推荐用菜单或 `lq forward` 命令，不要手写 TSV。Leikwan Toolkit 1.1.1 的转发模型是：
...
DDNS 刷新只检查 enabled 转发目标中的域名后端。IP 变化时会更新 `resolved.tsv`，创建自动快照，并安全重应用 nftables；IP 未变化时不会重复 apply。
