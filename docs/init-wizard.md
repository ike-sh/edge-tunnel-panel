...
Leikwan Toolkit 1.3.5 新增 `lq init`，适合首次部署、重装恢复和不确定当前机器角色时使用。
...
1.3.5 起，角色检测不会把 B 机器上的 `entries.tsv` 当成 entry 角色。只有 entry service、`ROLE=cloud-entry` 或 entry env 这类 A 侧信号存在时，才识别为公网入口。
