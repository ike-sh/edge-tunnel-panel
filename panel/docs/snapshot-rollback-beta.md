...
Leikwan Panel `3.0.0-alpha.4` adds a Controller-side safety framework for Plans.

Leikwan Panel 3.0.0-alpha.4 records manual snapshot and rollback information. It does not create snapshots, does not restore snapshots, does not roll back nodes, and does not change Leikwan Core configuration.
...
Future write automation must have an auditable recovery path before it can be considered safe. Leikwan Panel 3.0.0-alpha.4 establishes the fields, UI and API shape without enabling writes.

## 3.0.0-alpha.4 Action Review Integration

`3.0.0-alpha.4` adds action review metadata on top of this framework. Snapshot and rollback fields are treated as required gates for future write actions such as `create_entry`, `create_forward`, and `switch_entry`.
...
This document remains under its original filename for link compatibility. The snapshot and rollback metadata model described here is part of Leikwan Panel 3.0.0-alpha.4.
