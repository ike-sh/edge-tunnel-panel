...
const writeExecutionDisabledReason = "node write execution is limited to fixed 3.0.0-alpha.4 allowlisted actions; arbitrary command dispatch remains disabled"
...
func actionCatalogDefinitions() []ActionDefinition {
	...
	return []ActionDefinition{
		...
		readonlyAction("verify_config", "Verify config", "Readonly Agent config verification."),
		{
			...
			RiskLevel:             "high",
			Description:           "Future guarded write action for adding a public entry. Disabled in 3.0.0-alpha.4; Web apply uses fixed lower-level actions instead.",
		...
		},
		{
			...
			Category:              "future_write_guarded",
			RiskLevel:             "medium",
			Description:           "Future guarded write action for adding a forward target. Disabled in 3.0.0-alpha.4; Web apply uses fixed lower-level actions instead.",
			RequiredGates:         append([]string(nil), futureBaseGates...),
		...
		},
		{
			...
			RiskLevel:             "critical",
			Description:           "Future dangerous write action for changing PRIMARY/BACKUP entry state. Disabled in 3.0.0-alpha.4.",
			RequiredGates:         append([]string(nil), futureBaseGates...),
		...
		},
		{
			...
			RiskLevel:             "medium",
			Description:           "Future guarded write action for changing DDNS configuration. Disabled in 3.0.0-alpha.4.",
			RequiredGates:         append([]string(nil), futureBaseGates...),
		...
		},
		{
			...
			Category:              "future_write_dangerous",
			RiskLevel:             "critical",
			Description:           "Future dangerous action for restoring prior configuration. Disabled in 3.0.0-alpha.4.",
		...
		},
		{
			...
			Category:              "future_write_dangerous",
			RiskLevel:             "high",
			Description:           "Future dangerous action for restarting EasyTier relay. Disabled in 3.0.0-alpha.4.",
