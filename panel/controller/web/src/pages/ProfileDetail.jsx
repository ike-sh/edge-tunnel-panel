...
export default function ProfileDetail({
  ...
  async function handleDiagnose() {
    setDiagLoading(true);
    try {
      const output = await onDiagnose(profile);
      setDiagModal({ content: output || '（诊断完成，无输出）' });
    } catch (error) {
  ...
  return (
    ...
    <div className="page-stack">
      ...
      <Card
        ...
        actions={(
          ...
          <div className="head-actions">
            ...
            <button type="button" className="secondary small" onClick={handleDiagnose} disabled={loading || syncing || diagLoading}>{diagLoading ? '诊断中…' : '诊断'}</button>
      ...
      <CopyModal
        open={!!diagModal}
        title="线路诊断结果"
        description="来自 ix_read_diagnose 任务输出。"
        content={diagModal?.content || ''}
        onClose={() => setDiagModal(null)}
        onCopy={(text) => onCopy?.(text, '诊断结果')}
      />
