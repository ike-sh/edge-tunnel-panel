...
const DEFAULT_VERSION = 'v0.2.1-hotfix';

const tabs = [['dashboard', '总览'], ['nodes', '节点'], ['networks', '组网配置'], ['entries', '公网入口'], ['forwards', '转发规则'], ['pbr', '出口策略'], ['tasks', '任务'], ['settings', '设置']];
const checkActions = ['run_node_preflight', 'collect_agent_status', 'verify_agent_config', 'verify_easytier_status', 'verify_forward_rules', 'verify_pbr_rules', 'verify_ddns_status'];
...
function App() {
  ...
  async function quickApplyNetwork(event) {
    ...
    await run('创建并应用组网', async () => {
      ...
      await refreshTasks();
      setActiveTab('tasks');
      setAlert({ type: 'success', message: '已创建入口节点和后端节点组网任务，请到“任务”页面按节点筛选查看结果。' });
    });
  }
...
function TaskOutput({ task, onCopy }) {
  ...
  return <details open={task.status === 'failed'}><summary>{task.status === 'failed' ? '查看错误' : '查看详情'}</summary>{/no space left on device|磁盘空间不足/i.test(content) && <div className="alert error">该节点磁盘空间不足，建议换更大磁盘节点测试，或清理系统后重试。</div>}{onCopy && <button className="secondary mini" onClick={() => onCopy(content)}>复制输出</button>}{['apply_network_profile', 'verify_easytier_status'].includes(task.action) && <div className="task-kv"><span>config_path：{parsedResult.config_path || '-'}</span><span>service_path：{parsedResult.service_path || '-'}</span><span>binary_path：{parsedResult.binary_path || '-'}</span><span>easytier_status：{parsedResult.easytier_status || '-'}</span><span>peer_count：{parsedResult.peer_count ?? '-'}</span><span>has_remote_peer：{String(parsedResult.has_remote_peer ?? '-')}</span><span>listeners：{(parsedResult.listeners || []).join(', ') || '-'}</span><span>peers：{(parsedResult.peers || []).join(', ') || '-'}</span></div>}{parsedResult.peer_info && <pre className="small task-output">peer_info:{'\n'}{parsedResult.peer_info}</pre>}<pre className="small task-output">{`task id: ${task.id}\nnode id: ${task.node_id || '-'}\naction: ${task.action}\naction label: ${actionText[task.action] || task.action}\nstatus: ${task.status}\ncreated_at: ${task.created_at || '-'}\nstarted_at: ${task.started_at || '-'}\nfinished_at: ${task.finished_at || '-'}\n\npayload:\n${payload}\n\nresult:\n${result}\n\nstdout:\n${stdout}\n\nstderr:\n${stderr}\n\nerror:\n${error}`}</pre></details>;
}
