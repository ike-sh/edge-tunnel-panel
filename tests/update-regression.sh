...
#!/usr/bin/env bash
case "${1:-}" in
  --version|-v) echo "leikwan-toolkit 1.3.4" ;;
  *) exit 0 ;;
esac
EOF
...
LAST_UPDATE_FROM=1.3.3
LAST_UPDATE_TO=1.3.4
LAST_UPDATE_RESULT=ok
LAST_UPDATE_BACKUP=${TMP_DIR}/backup.sh
LAST_UPDATE_SOURCE=https://github.com/ike-sh/leikwan-toolkit/releases/download/v1.3.4/leikwan-toolkit-1.3.4.tar.gz
LAST_UPDATE_VERSION=1.3.4
EOF

check_out="$(update_check 2>&1)"
grep -q "当前安装版本：1.3.4" <<<"$check_out"
grep -q "当前运行进程：1.3.3" <<<"$check_out"
grep -q "最新版本：1.3.4" <<<"$check_out"
grep -q "当前已是最新版本：1.3.4" <<<"$check_out"
...
grep -q "当前运行版本: 1.3.3" <<<"$status_out"
grep -q "当前安装版本: 1.3.4" <<<"$status_out"
grep -q "快捷命令: ${shortcut_lq} -> ${installed_script}" <<<"$status_out"
grep -q "最近更新: 1.3.3 -> 1.3.4 / OK" <<<"$status_out"
grep -q "建议重新进入菜单：lq" <<<"$status_out"

export LEIKWAN_DISABLE_UPDATE_EXEC=1
UPDATE_RELOAD_AFTER_ACTION=1
reload_out="$(update_maybe_reload_after_change 1.3.4 update 2>&1)"
grep -q "当前菜单进程仍是旧版本，正在重新载入新版本" <<<"$reload_out"
grep -q "已跳过自动 exec" <<<"$reload_out"

rollback_out="$(update_maybe_reload_after_change 1.3.4 rollback 2>&1)"
grep -q "当前菜单进程仍是回滚前版本，正在重新载入" <<<"$rollback_out"

UPDATE_RELOAD_AFTER_ACTION=0
non_menu_out="$(update_maybe_reload_after_change 1.3.4 update 2>&1)"
