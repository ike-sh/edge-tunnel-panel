...
before="$(find "$LEIKWAN_STATE_DIR" -type f -printf '%P %s\n' | sort)"
out="$(ddns_check_consistency)"
after="$(find "$LEIKWAN_STATE_DIR" -type f -printf '%P %s\n' | sort)"

grep -q "DDNS 一致性检查" <<<"$out"
...
grep -q "B entries:" <<<"$empty_out"
grep -q "未配置" <<<"$empty_out"
grep -q "A entry:" <<<"$empty_out"
